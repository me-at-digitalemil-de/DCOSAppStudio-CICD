...
function createDashboard() {
  for(let vi= 1; vi< vis.length; vi++) {
  let v= JSON.stringify(vis[vi-1]);
  v= v.replace(/PIVOTFIELD/g, fields[pivot]);
  v= v.replace(/__INDEX/g, appdef.path);
  console.log("v"+vi+": "+v);
  putToElastic(".kibana",JSON.parse(v), "visualization", "v"+vi);
  }
